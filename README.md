...
문서는 이 폴더에서 검토한 뒤 공개 저장소로 한 번에 이전합니다. 이전 시 Mintlify로 배포하고, 사람과 Agent가 같은 문서를 각각 웹·Markdown·`llms.txt`·read-only MCP 검색으로 이용하게 합니다.
...
Portal의 한국어 i18n을 바꾼 뒤에는 저장소 루트에서 아래 순서로 실행합니다.
